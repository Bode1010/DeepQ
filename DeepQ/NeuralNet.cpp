...
float defaultCost(float input, float target) {
	return input - target;
}
...
void NeuralNet::BackPropagate(const vector<float>& output, int pipe) {
	...
	//For every layer except the input. update weight and neurons
	for (int i = outLayIndex; i > 0; i--) {
		if (net[i].getLayerType() == DENSE) DenseBackPass(i, pipe);
	}

}

float NeuralNet::CostDerivative(float input, float target) {
	float result = (*cost)(input, target) * net[net.size() - 1].dActivate(input);
	return result;
}
...
void NeuralNet::DenseBackPass(int layerIndex, int pipe) {
	//for every neuron
		...
		if (net[layerIndex].neuron[i].getActive(pipe)) {
			...
			if (layerIndex != net.size() - 1 ){//&& !(layerIndex != net.size() - 1 && i == net[layerIndex].size() - 1)) {//except the last layer and the bias
				for (int j = 0; j < net[layerIndex + 1].size(); j++) {
					...
					if (layerIndex == net[layerIndex].size() - 1 || j != net[layerIndex + 1].size() - 1) {
...
void NeuralNet::SetCostFuncDerivative(float (*func)(float, float), float a, float b) {
	cost = func;
}
